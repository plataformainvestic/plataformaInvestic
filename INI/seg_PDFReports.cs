          ...
          public static MemoryStream Reporte(Reporte repodata)
            {
                ...
                List<tblActividadContratista> data = new List<tblActividadContratista>();
                data = db.tblActividadContratista.Where(x => x.Id_Contratista.Equals(repodata.UserId)).ToList();
                List<tblProductosContratista> prolist = new List<tblProductosContratista>();
                List<tblEvidenciasContratista> evilist = new List<tblEvidenciasContratista>();
                List<tblResponsabContratista> contra = new List<tblResponsabContratista>();
                contra = db.tblResponsabContratista.Where(x=>x.Id_Contratista.Equals(repodata.UserId)).ToList();
                ...
                string cedu = (from c in db.AspNetUsers where c.Id.Equals(repodata.UserId) select c.PersonalID).SingleOrDefault();
                ...
                string equip = (from e in db.tblEquiposTrabajo where e.Id_Equipo == ecc select e.Nombre_Equipo).SingleOrDefault();
                string nombrecoord = (from e in db.tblEquiposTrabajo where e.Id_Equipo == ecc select e.Nombre_Coordinador).SingleOrDefault();

                string nom = (from n in db.AspNetUsers where n.Id.Equals(repodata.UserId) select n.Name).SingleOrDefault();
                string ape = (from a in db.AspNetUsers where a.Id.Equals(repodata.UserId) select a.SureName).SingleOrDefault();
                ...
                seg_pdfPage PageEventHandler = new seg_pdfPage();
                    ...
                    foreach (var actividad in data)
                    {
                        if (actividad.Fecha_Ini >= repodata.Fechainicial)
                        {
                            if (actividad.Fecha_Fin <= repodata.FechaFinal)
                            {
                                ...
                                pdfTab.AddCell(actividad.tblEstadoTarea.Des_Estado);

                                prolist = db.tblProductosContratista.Where(x => x.Id_Contratista.Equals(repodata.UserId)).Where(i => i.Id_Actividad == actividad.Id_Actividad).ToList();
                                ...
                                evilist = db.tblEvidenciasContratista.Where(x => x.Id_Contratista.Equals(repodata.UserId)).Where(i => i.Id_Actividad == actividad.Id_Actividad).ToList();
                ...
                if (repodata.Tipo == 0)
                {
                     ...
                     foreach (var actividad in data)
                     {
                         if (actividad.Fecha_Ini >= repodata.Fechainicial)
                         {
                             if (actividad.Fecha_Fin <= repodata.FechaFinal)
                             {
                                 pdfTab.AddCell(actividad.tblAlternativas.Id_Alternativa.ToString());
                                 pdfTab.AddCell(actividad.tblResponsabContratista.IdentificadorResponsa);
